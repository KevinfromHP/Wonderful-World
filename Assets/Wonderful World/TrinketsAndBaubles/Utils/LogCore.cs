...
using System.Runtime.CompilerServices;
    ...
    public static class LogCore
    {
        ...
        {
            logger.LogDebug(string.Format("TrinketsAndBaubles :: {0} :: Line: {1}, Method {2}", data, i, member));
        }
        public static void LogE(object data, [CallerLineNumber] int i = 0, [CallerMemberName] string member = "")
        {
            logger.LogError(string.Format("TrinketsAndBaubles :: {0} :: Line: {1}, Method {2}", data, i, member));
        }
        ...
        {
            logger.LogInfo(string.Format("TrinketsAndBaubles :: {0} :: Line: {1}, Method {2}", data, i, member));
        }
        public static void LogM(object data, [CallerLineNumber] int i = 0, [CallerMemberName] string member = "")
        {
            logger.LogMessage(string.Format("TrinketsAndBaubles :: {0} :: Line: {1}, Method {2}", data, i, member));
        }
        public static void LogW(object data, [CallerLineNumber] int i = 0, [CallerMemberName] string member = "")
        {
            logger.LogWarning(string.Format("TrinketsAndBaubles :: {0} :: Line: {1}, Method {2}", data, i, member));
        }
    }
