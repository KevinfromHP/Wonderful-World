...
namespace TrinketsAndBaubles.Utils
{
    public static class CloudUtils
    {
        ...
        /// <summary>
        /// Adds an ItemDef to the item catalog and returns true
        /// ItemDef must be non-null
        /// returns false if ItemDef is null     
        /// </summary>
        /// <param name="item">The item to register to the item catalog.</param>
        /// <returns></returns>
        public static bool RegisterNewItem(ItemDef item)
        {
            if (item)
            {
                EnigmaticThunder.Modules.Pickups.RegisterItem(item);
                LogCore.LogD("Registered item " + item.nameToken + " to the equipment catalog!");
                return true;
            }
            LogCore.LogF("FATAL ERROR: item failed to register to the item catalog because it is null!");
            return false;
        }
        /// <summary>
        /// Adds an EquipmentDef to the equipment catalog and returns true
        /// EquipmentDef must be non-null
        /// returns false if EquipmentDef is null     
        /// </summary>
        /// <param name="equip">The equipment to register to the equipment catalog.</param>
        /// <returns></returns>
        public static bool RegisterNewEquipment(EquipmentDef equip)
        {
            if (equip)
            {
                EnigmaticThunder.Modules.Pickups.RegisterEquipment(equip);
                LogCore.LogD("Registered equip " + equip.nameToken + " to the equipment catalog!");
                return true;
            }
            LogCore.LogF("FATAL ERROR: equip failed to register to the equip catalog because it is null!");
            return false;
        }
